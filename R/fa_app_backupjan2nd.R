...
setup_twitter_oauth("LOCY7w4sCs14rIevtYozJyCAL","K43I4OcyBsLRhyqP90KMdodSPI6ZCwErSdvGvq56l9PEVlCTd1","812924022287634432-KnmwBoiGPtKm6NCjU5hiCVcb3ScCgqD","AkBpQ6FJ1QAJs7ZWaBKxTj190c6K1XK3SWGCszznFnYMt")
  ...
  fluidPage({
    ...
    sidebarLayout(
      ...
      mainPanel(
        ...
        h3("Who Signed Where?"),
        tableOutput("signed"),
        h3("Bidding history of FA in target"),
        tableOutput("table"),
        h3("Your Team Bidding Progress (BETA Version)"),
        tableOutput("sort_by_team"),
        h3("10 Most Recent Bidding (Most recent bid at the top. Time in ET)"),
        tableOutput("recent"),
        h3("Highest bidding point for each of 2016 FA. (Time in ET)"),
...
server <- shinyServer(function(input, output, session){
  ...
  sliderValues <- eventReactive(input$choose,{
    ...
    if ((USER$Logged == TRUE) & (input$Username != "Tony") & (finished == "NO")){
      ...
      if((success == TRUE) & (input$Username != "Tony")){
        ...
        tbl55 <- dbReadTable(con,"Password",row.names=NULL)
        ...
        if(all(points > check))
        {
          ...
          for(jack in 1:length(teams_to_send_email))
          {
            ...
            body <- paste("Hello ",namer,", You have been outbid by an unknown team for: ",input$FA_players,". ",
                          "An unknown club bidded ",test$Points[1], " points for the lead. Challenge that bid by following this link: https://wsfbdraft.shinyapps.io/fa_center/",
                          " Thank you.
            ...
            send.mail(from = "taeheeryu263@gmail.com",
                      to = recipients,
                      subject = paste("You have been outbid by somebody for the following FA: ",input$FA_players,sep=""),
                      body = body,
                      smtp = list(host.name = "smtp.gmail.com", port = 587, user.name = "taeheeryu263", passwd = "ripken1234", tls = TRUE),
                      authenticate = TRUE,
                      send = TRUE)
          }
        }
  ...
  recent_email <- reactive({
    ...
    for(m in 1:nrow(clock))
    {
      ...
      if(length(which(tbl7$Player %in% clock$Player[m])) > 0)
      {
        ...
        if(((tbl7$Time_Left[which(tbl7$Player %in% clock$Player[m])]) <= 0) == TRUE)
        {
          # "NO" assigned if email was already sent out before and it has more than 1 hour left.
          # "YES" assigned if email was not sent out before and it has more than 1
          ifelse((clock$clockend[m] == "YES") == TRUE,clock$send[m] <- NA,clock$send[m] <- 0)
    ...
    if(length(mail_out) > 0)
    {
      ...
      clock$send[which(clock$send %in% c(0,"0"))] <- NA
  ...
  output$sort_by_team <- renderTable({
    ...
    if(USER$Logged == TRUE){
      ...
      tbl56 <- dbReadTable(con,"Password",row.names=NULL)
      
      tbl10 <- dbReadTable(con,"FA_TABLE",row.names=NULL)
  ...
  output$downloadData <- downloadHandler(
    ...
    content = function(filename,results) {
      ...
      clocked <- dbReadTable(con,"clock2",row.names=NULL)
      
      clocked_name <- clocked$Player[clocked$clockend %in% "YES"]
      
      results <- dbReadTable(con,"FA_TABLE",row.names=NULL)
      
      total_dollar_table <- dbReadTable(con,"total_dollar_table",row.names = NULL)
      
      results <- results[,c("Player","Club","Year_Guaranteed","summary","Signing_Bonus","X2017","X2018","X2019","X2020","X2021","X2022","X2023","X2024","X2025","X2026","Club_Option","Buyout_1","Vesting","Buyout2","Points","AAV","Total","Contract_Status")]
      ...
      dbWriteTable(conn=con,"total_dollar_table",total_dollar_table,overwrite=TRUE)
  ...
  output$downloadData2 <- downloadHandler(
    ...
    content = function(filename,tables) {
      ...
      if(USER$Logged == TRUE){
        ...
        tables <- dbReadTable(con,"FA_TABLE",row.names=NULL)
