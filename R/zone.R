...
date <- seq(from=as.Date("2016-04-03",format="%Y-%m-%d",origin="1970-01-01"), to=as.Date("2016-06-19",format="%Y-%m-%d",origin="1970-01-01"), by = 1)
date11 <- strftime(strptime(date, "%Y-%m-%d"), "%Y%m%d")

# Convert date to the format that is required for loading BIS data

date2 <- seq(from=as.Date("2016-04-04",format="%Y-%m-%d",origin="1970-01-01"), to=as.Date("2016-06-20",format="%Y-%m-%d",origin="1970-01-01"), by = 1)
date22 <- strftime(strptime(date2, "%Y-%m-%d"), "%Y%m%d")

master_column <- c("MLBId","PlayerName","Zone","Block","Frame","Team","Pos","INN")

master_ytd <- data.frame(matrix(NA,nrow=1,ncol=8))

colnames(master_ytd) <- master_column
...
YTD2 <- read.csv(paste("BIS/YTDFielding_",date22[i],".csv",sep=""))
YTD <- read.csv(paste("BIS/YTDFielding_",date11[i],".csv",sep=""))

# Characterize columns

ytd_col <- c("LastName","FirstName","MLBId","PlayerName")

for(j in 1:length(ytd_col))
{
  YTD[,ytd_col[j]] <- as.character(YTD[,ytd_col[j]])
  YTD2[,ytd_col[j]] <- as.character(YTD2[,ytd_col[j]])
...
YTD2 <- YTD2[,c("MLBId","PlayerName","Zone","Block","Frame","Team","Pos","INN")]

master_ytd <- rbind(master_ytd,YTD2) 

}

master_ytd <- master_ytd[!master_ytd$Zone == "",]

master_ytd$Zone <- as.numeric(master_ytd$Zone)

UZR <- master_ytd %>% group_by(MLBId,PlayerName) %>% summarize(total_zone = sum(Zone))

UZR$Team <- ""

position <- read.csv("playerid_with_pos.csv")

for(j in 1:nrow(UZR))
{
  if(TRUE %in% (position$MLBId %in% UZR$MLBId[j]))
  {
   UZR$Team[j] <- as.character(position$Team_RFB[which(position$MLBId %in% UZR$MLBId[j])])
  }
  
  if(FALSE %in% (position$MLBId %in% UZR$MLBId[j]))
  {
    next;
  }
}

UZR <- UZR[order(UZR$total_zone,decreasing = TRUE),]